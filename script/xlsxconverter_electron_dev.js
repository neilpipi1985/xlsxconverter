...
async function run() {
  ...
  try {
    ...
    await XlsxConverter.initApp({ enableDevMode: true, appPath });
...
function createWindow() {
  if (!mainWindow) {
    ...
    run().then((msg = '') => {
      ...
      mainWindow.loadURL(`data:text/html, ${XlsxConverter.defaultRender(process.env.ELECTRON_RENDER_FILE)}`, {
...
StationMgr.SocketMgr.addSocket('render', new RenderEmitter());
StationMgr.injectSocketEvent(ipcMain);

StationMgr.SocketMgr.on(ipcMain, 'setPath', async (...para) => {
  const obj = SocketMgr.getSocketPara(para, socket);
  const opts = obj.opts || {};
  const logPath = await StationMgr.logPath();
  ...
  if (dir) {
    await StationMgr.setSysSetting({ stationOpts: { logPath: dir[0] } });
  }
});

StationMgr.SocketMgr.on(ipcMain, 'exitApp', async () => {
  app.quit();
});
